  ...
  Delta apply(Delta document, int index, Object data) {
    ...
    final hasLinkOrMention = (attributes != null &&
        (attributes.containsKey(NotusAttribute.link.key) ||
            attributes.containsKey(NotusAttribute.mentionPerson.key) ||
            attributes.containsKey(NotusAttribute.mentionTopic.key) ||
            attributes.containsKey(NotusAttribute.mentionPost.key)));
    if (!hasLinkOrMention) {
      return Delta()
        ..retain(index)
    ...
    noLinkAndMentionAttributes.remove(NotusAttribute.mentionPerson.key);
    noLinkAndMentionAttributes.remove(NotusAttribute.mentionTopic.key);
    noLinkAndMentionAttributes.remove(NotusAttribute.mentionPost.key);
    ...
    if (!(nextAttributes.containsKey(NotusAttribute.link.key) ||
        nextAttributes.containsKey(NotusAttribute.mentionPerson.key) ||
        nextAttributes.containsKey(NotusAttribute.mentionPost.key) ||
        nextAttributes.containsKey(NotusAttribute.mentionTopic.key))) {
    ...
    if (nextAttributes.containsKey(NotusAttribute.mentionPerson.key)) {
      return Delta()
        ...
        ..insert(previousText)
        ..insert(text, attributes.remove(NotusAttribute.mentionPerson))
        ..delete(next.length)
        ..insert(next.data);
    }
    if (nextAttributes.containsKey(NotusAttribute.mentionPost.key)) {
      return Delta()
        ...
        ..delete(previousText.length)
        ..insert(previousText)
        ..insert(text, attributes.remove(NotusAttribute.mentionPost))
        ..delete(next.length)
        ..insert(next.data);
    }
    if (nextAttributes.containsKey(NotusAttribute.mentionTopic.key)) {
      return Delta()
        ...
        ..delete(previousText.length)
        ..insert(previousText)
        ..insert(text, attributes.remove(NotusAttribute.mentionTopic))
        ..delete(next.length)
        ..insert(next.data);
    }
...
class InsertDateRule extends InsertRule {
  const InsertDateRule();
  ...
  Delta apply(Delta document, int index, Object data) {
    ...
    final attributes = previous?.attributes ?? {};
    if (attributes.containsKey(NotusAttribute.date.key)) {
      result..insert(' ');
    }

    /// check if the insertion is date
    if (data as String == '!-Date-!') {
      result..insert('  ');
    } else {
      result..insert(data);
    }
