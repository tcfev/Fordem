...
import 'package:notus/src/models/image_data.dart';
import 'package:notus/src/models/location_data.dart';
import 'package:notus/src/models/video_data.dart';
...
// rpedeTODO: document model currently only supports BlockEmbeds and need to be updated to support SpanEmbeds.
// TODO: @Arman bring the date Embed here
...
///
/// There are two built-in embed types supported by Notus documents, however
/// the document model itself does not make any assumptions about the types
/// of embedded objects and allows users to define their own types.
///
/// It is also allows to re-define the built-in embed types (horizontal rule
...
class BlockEmbed extends EmbeddableObject {
  ...
  static BlockEmbed image(ImageData data) =>
      BlockEmbed('image', data: data.toJson());

  static BlockEmbed video(VideoData data) =>
      BlockEmbed('video', data: data.toJson());

  static BlockEmbed file(FileData data) =>
      BlockEmbed('file', data: data.toJson());

  static BlockEmbed audio(AudioData data) =>
      BlockEmbed('audio', data: data.toJson());

  static BlockEmbed location(LocationData data) =>
      BlockEmbed('location', data: data.toJson());

  static BlockEmbed table(List<List<String>> data) =>
      BlockEmbed('table', data: {'data': data});
}
