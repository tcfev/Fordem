...
class Operation {
  ...
  /// Payload of "insert" operation. For other types it is set to empty string.
  final Object data;

  /// Rich-text attributes are set by this operation. It can be `null`.
  ...
  /// Creates an Operation which deletes [length] of characters.
  factory Operation.delete(int length) =>
      Operation._(Operation.deleteKey, length, '', null);

  /// Creates an Operation which inserts [text] with optional [attributes].
  ...
  ///
  /// For insert operations this returns text, for delete and retain, the length.
  ...
  /// Returns `true` if this operation is empty.
  ///
  /// An operation is considered empty if its [length] is equal to `0`.
  bool get isEmpty => length == 0;

  /// Returns `true` if this operation is not empty.
...
class Delta {
  ...
  ///get anti-attr result based on base
